...
namespace coralstock
{
    public class Importer : Parser
    {
        ...
        Workbook xlLookup = new Workbook();
        Worksheet oSheet, xlWorkSheet;
        ...
        public Importer()
        {
            ...
            string filename = Path.GetDirectoryName(path )+ @"\data\Coral Aquarium Conditions.xls";
            xlWorkBook = Workbook.Load(filename);
            string lookupfile = Path.GetDirectoryName(path) + @"\data\Category SKU Match.xls";
            xlLookup = Workbook.Load(lookupfile);

            xlWorkSheet = xlLookup.Worksheets[0];
            for (int rCnt = 0; rCnt <= xlWorkSheet.Cells.LastRowIndex-1; rCnt++)
            {
                string RefID = xlWorkSheet.Cells[rCnt, 0].StringValue;
                string catIDs = xlWorkSheet.Cells[rCnt, 1].StringValue;
        ...
        private Worksheet getWorksheetByName(Workbook xlworkBook,string name)
        {
            foreach (Worksheet xlWS in xlworkBook.Worksheets)
            {
                if (xlWS.Name.ToLower() == name.ToLower())
                    return xlWS;
            }
            return null;
        }
        public string[] getDescriptions()
        {
            ...
            try
            {                
                Worksheet xlWorkSheet = getWorksheetByName(xlWorkBook,Category);
                careLevel = xlWorkSheet.Cells[1, 2].StringValue;
                placement = xlWorkSheet.Cells[2, 2].StringValue;
                lighting = xlWorkSheet.Cells[3, 2].StringValue;
                flow = xlWorkSheet.Cells[4, 2].StringValue;
                temperature = xlWorkSheet.Cells[5, 2].StringValue;
                salinity = xlWorkSheet.Cells[6, 2].StringValue;
                calcium = xlWorkSheet.Cells[7, 2].StringValue;
                alkalinity = xlWorkSheet.Cells[8, 2].StringValue;
                magnesium = xlWorkSheet.Cells[9, 2].StringValue;
            }
            catch (Exception ex)
            {
            }
